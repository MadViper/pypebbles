...
from pypebbles.fluent import FluentDict


def test_should_construct() -> None:
    assert FluentDict[Any]().with_a(a=1).and_a(b=2).and_a(c=3) == {
        "a": 1,
        "b": 2,
        "c": 3,
    }
...
def test_should_extract_value() -> None:
    assert FluentDict[Any]().with_a(a=1).value_of("a").to(int) == 1
